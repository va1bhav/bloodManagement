...
- (void) show:(CDVInvokedUrlCommand *)command
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (@available(iOS 13.0, *)) {
            barcodeView.modalInPresentation = YES;
        }
...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    /**
        For fixing the style break after clicking the cancel button
     */
   [self.barcodeCamera.webView setFrame:CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height)];
...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator
{
    ...
    [_prevLayer.connection setVideoOrientation:[self avOrientationForDeviceOrientation:[[UIDevice currentDevice] orientation]]];
        NSLog(@"vimal 1%f", [[UIScreen mainScreen] bounds].size.width);
    NSLog(@"vimal 2%f", self.view.frame.size.width);
    NSLog(@"vimal 3%f", size.width);
    ...
    [super viewWillTransitionToSize:size withTransitionCoordinator:coordinator];
      ...
      {
        AVCaptureVideoPreviewLayer *previewLayer = _prevLayer;
